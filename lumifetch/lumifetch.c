#ifndef LUMIFETCH_VERSION
	#define LUMIFETCH_VERSION "unkown" // needs to be changed by the build script
#endif
...
struct configuration parse_config(struct info* user_info, struct user_config* user_config_file) {
	...
	if (user_config_file->config_directory == NULL) { // if config directory is not set, try to open the default
		if (getenv("HOME") != NULL) {
			char homedir[512];
			sprintf(homedir, "%s/.config/lumifetch/config", getenv("HOME"));
			LOG_V(homedir);
			config = fopen(homedir, "r");
			if (!config) {
				if (getenv("PREFIX") != NULL) {
					char prefixed_etc[512];
					sprintf(prefixed_etc, "%s/etc/lumifetch/config", getenv("PREFIX"));
					LOG_V(prefixed_etc);
					config = fopen(prefixed_etc, "r");
				} else
					config = fopen("/etc/lumifetch/config", "r");
			}
		}
	} else
...
int print_image(struct info* user_info) {
	...
	if (strlen(user_info->image_name) < 1) {
		char* repl_str = strcmp(user_info->os_name, "android") == 0 ? "/data/data/com.termux/files/usr/lib/lumifetch/%s.png"
										 : strcmp(user_info->os_name, "macos") == 0 ? "/usr/local/lib/lumifetch/%s.png"
																																: "/usr/lib/lumifetch/%s.png";
...
#define STRING_TO_OWO(original, uwufied) \
	if (strcmp(user_info->os_name, original) == 0) sprintf(user_info->os_name, "%s", uwufied)
	// linux
	STRING_TO_OWO("alpine", "Nyalpine");
	else STRING_TO_OWO("amogos", "AmogOwOS");
	else STRING_TO_OWO("android", "Nyandroid");
	else STRING_TO_OWO("arch", "Nyarch Linuwu");
	else STRING_TO_OWO("arcolinux", "ArcOwO Linuwu");
	else STRING_TO_OWO("artix", "Nyartix Linuwu");
	else STRING_TO_OWO("debian", "Debinyan");
	else STRING_TO_OWO("devuan", "Devunyan");
	else STRING_TO_OWO("deepin", "Dewepyn");
	else STRING_TO_OWO("endeavouros", "endeavOwO");
	else STRING_TO_OWO("EndeavourOS", "endeavOwO");
	else STRING_TO_OWO("fedora", "Fedowa");
	else STRING_TO_OWO("femboyos", "FemboyOWOS");
	else STRING_TO_OWO("gentoo", "GentOwO");
	else STRING_TO_OWO("gnu", "gnUwU");
	else STRING_TO_OWO("guix", "gnUwU gUwUix");
	else STRING_TO_OWO("linuxmint", "LinOWO Miwint");
	else STRING_TO_OWO("manjaro", "Myanjawo");
	else STRING_TO_OWO("manjaro-arm", "Myanjawo AWM");
	else STRING_TO_OWO("neon", "KDE NeOwOn");
	else STRING_TO_OWO("nixos", "nixOwOs");
	else STRING_TO_OWO("opensuse-leap", "OwOpenSUSE Leap");
	else STRING_TO_OWO("opensuse-tumbleweed", "OwOpenSUSE Tumbleweed");
	else STRING_TO_OWO("pop", "PopOwOS");
	else STRING_TO_OWO("raspbian", "RaspNyan");
	else STRING_TO_OWO("rocky", "Wocky Linuwu");
	else STRING_TO_OWO("slackware", "Swackwawe");
	else STRING_TO_OWO("solus", "sOwOlus");
	else STRING_TO_OWO("ubuntu", "Uwuntu");
	else STRING_TO_OWO("void", "OwOid");
	else STRING_TO_OWO("xerolinux", "xuwulinux");

	// BSD
	else STRING_TO_OWO("freebsd", "FweeBSD");
	else STRING_TO_OWO("openbsd", "OwOpenBSD");

	// Apple family
	else STRING_TO_OWO("macos", "macOwOS");
	else STRING_TO_OWO("ios", "iOwOS");

	// Windows
	else STRING_TO_OWO("windows", "WinyandOwOws");
...
#define KERNEL_TO_OWO(str, original, uwufied) \
	...
	for (int i = 0; i < 16; i++) {
		...
		KERNEL_TO_OWO(splitted[i], "Linux", "Linuwu");
		else KERNEL_TO_OWO(splitted[i], "linux", "linuwu");
		else KERNEL_TO_OWO(splitted[i], "alpine", "Nyalpine");
		else KERNEL_TO_OWO(splitted[i], "amogos", "AmogOwOS");
		else KERNEL_TO_OWO(splitted[i], "android", "Nyandroid");
		else KERNEL_TO_OWO(splitted[i], "arch", "Nyarch Linuwu");
		else KERNEL_TO_OWO(splitted[i], "artix", "Nyartix Linuwu");
		else KERNEL_TO_OWO(splitted[i], "debian", "Debinyan");
		else KERNEL_TO_OWO(splitted[i], "deepin", "Dewepyn");
		else KERNEL_TO_OWO(splitted[i], "endeavouros", "endeavOwO");
		else KERNEL_TO_OWO(splitted[i], "EndeavourOS", "endeavOwO");
		else KERNEL_TO_OWO(splitted[i], "fedora", "Fedowa");
		else KERNEL_TO_OWO(splitted[i], "femboyos", "FemboyOWOS");
		else KERNEL_TO_OWO(splitted[i], "gentoo", "GentOwO");
		else KERNEL_TO_OWO(splitted[i], "gnu", "gnUwU");
		else KERNEL_TO_OWO(splitted[i], "guix", "gnUwU gUwUix");
		else KERNEL_TO_OWO(splitted[i], "linuxmint", "LinOWO Miwint");
		else KERNEL_TO_OWO(splitted[i], "manjaro", "Myanjawo");
		else KERNEL_TO_OWO(splitted[i], "manjaro-arm", "Myanjawo AWM");
		else KERNEL_TO_OWO(splitted[i], "neon", "KDE NeOwOn");
		else KERNEL_TO_OWO(splitted[i], "nixos", "nixOwOs");
		else KERNEL_TO_OWO(splitted[i], "opensuse-leap", "OwOpenSUSE Leap");
		else KERNEL_TO_OWO(splitted[i], "opensuse-tumbleweed", "OwOpenSUSE Tumbleweed");
		else KERNEL_TO_OWO(splitted[i], "pop", "PopOwOS");
		else KERNEL_TO_OWO(splitted[i], "raspbian", "RaspNyan");
		else KERNEL_TO_OWO(splitted[i], "rocky", "Wocky Linuwu");
		else KERNEL_TO_OWO(splitted[i], "slackware", "Swackwawe");
		else KERNEL_TO_OWO(splitted[i], "solus", "sOwOlus");
		else KERNEL_TO_OWO(splitted[i], "ubuntu", "Uwuntu");
		else KERNEL_TO_OWO(splitted[i], "void", "OwOid");
		else KERNEL_TO_OWO(splitted[i], "xerolinux", "xuwulinux");

		// BSD
		else KERNEL_TO_OWO(splitted[i], "freebsd", "FweeBSD");
		else KERNEL_TO_OWO(splitted[i], "openbsd", "OwOpenBSD");

		// Apple family
		else KERNEL_TO_OWO(splitted[i], "macos", "macOwOS");
		else KERNEL_TO_OWO(splitted[i], "ios", "iOwOS");

		// Windows
		else KERNEL_TO_OWO(splitted[i], "windows", "WinyandOwOws");
...
// owofies hardware names
void owo_hw(char* hwname) {
	LOG_I("owofing hardware")
#define HW_TO_OWO(original, owoified) replace_ignorecase(hwname, original, owoified);
...
// owofies package manager names
void owo_pkgman(char* pkgman_name) {
	LOG_I("owofing package managers")
#define PKGMAN_TO_OWO(original, owoified) replace_ignorecase(pkgman_name, original, owoified);
	...
	PKGMAN_TO_OWO("rpm", "rawrpm");
	PKGMAN_TO_OWO("snap", "snyap");
	PKGMAN_TO_OWO("zypper", "zyppew");
#undef PKGMAN_TO_OWO
}
...
int print_info(struct configuration* config_flags, struct info* user_info) {
	...
	if (config_flags->show.cpu)
		responsively_printf(print_buf, "%s%s%sCPOWO    %s%s", MOVE_CURSOR, NORMAL, BOLD, NORMAL, user_info->cpu_model);

	for (int i = 0; i < 256; i++) {
		if (config_flags->show_gpu[i])
			if (user_info->gpu_model[i][0])
				responsively_printf(print_buf, "%s%s%sGPOWO    %s%s", MOVE_CURSOR, NORMAL, BOLD, NORMAL, user_info->gpu_model[i]);
	}
	...
	if (config_flags->show.uptime) {
		...
		case 0 ... 3599:
			responsively_printf(print_buf, "%s%s%sOWOPTIME %s%lim", MOVE_CURSOR, NORMAL, BOLD, NORMAL, user_info->uptime / 60 % 60);
			break;
		case 3600 ... 86399:
			responsively_printf(print_buf, "%s%s%sOWOPTIME %s%lih, %lim", MOVE_CURSOR, NORMAL, BOLD, NORMAL, user_info->uptime / 3600, user_info->uptime / 60 % 60);
			break;
		default:
			responsively_printf(print_buf, "%s%s%sOWOPTIME %s%lid, %lih, %lim", MOVE_CURSOR, NORMAL, BOLD, NORMAL, user_info->uptime / 86400, user_info->uptime / 3600 % 24, user_info->uptime / 60 % 60);
		}
	}
	// clang-format off
...
void write_cache(struct info* user_info) {
	LOG_I("writing cache");
	char cache_file[512];
	sprintf(cache_file, "%s/.cache/lumifetch.cache", getenv("HOME")); // default cache file location
	LOG_V(cache_file);
...
int read_cache(struct info* user_info) {
	LOG_I("reading cache");
	char cache_file[512];
	sprintf(cache_file, "%s/.cache/lumifetch.cache", getenv("HOME")); // cache file location
	LOG_V(cache_file);
...
int print_ascii(struct info* user_info) {
	...
	if (!file) { // if the file does not exist in the local directory, open it from the installation directory
		if (strcmp(user_info->os_name, "android") == 0)
			sprintf(ascii_file, "/data/data/com.termux/files/usr/lib/lumifetch/ascii/%s.txt", user_info->os_name);
		else if (strcmp(user_info->os_name, "macos") == 0)
			sprintf(ascii_file, "/usr/local/lib/lumifetch/ascii/%s.txt", user_info->os_name);
		else
			sprintf(ascii_file, "/usr/lib/lumifetch/ascii/%s.txt", user_info->os_name);
		LOG_V(ascii_file);
...
void usage(char* arg) {
	...
	printf("Usage: %s <args>\n"
				 ...
				 "    -V, --version       prints the current lumifetch version\n"
#ifdef __DEBUG__
				 "    -v, --verbose       logs everything\n"
#endif
				 "    -w, --write-cache   writes to the cache file (~/.cache/lumifetch.cache)\n"
				 "    -r, --read-cache    reads from the cache file (~/.cache/lumifetch.cache)\n",
				 arg,
#ifndef __IPHONE__
				 BLUE,
#else
...
int main(int argc, char* argv[]) {
	...
	while ((opt = getopt_long(argc, argv, OPT_STRING, long_options, NULL)) != -1) {
		...
		case 'r':
			...
			break;
		case 'V':
			printf("lumifetch version %s\n", LUMIFETCH_VERSION);
			return 0;
#ifdef __DEBUG__
		case 'v':
			*verbose_enabled = true;
			LOG_I("version %s", LUMIFETCH_VERSION);
			break;
#endif
		case 'w':
