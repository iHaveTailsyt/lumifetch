...
void* get_pkg(void* argp) { // this is just a function that returns the total of installed packages
	...
	#endif
#else
	struct package_manager pkgmans[] = {{"/usr/local/bin/brew", "find $(brew --cellar 2>/dev/stdout) -maxdepth 1 -type d 2> /dev/null | wc -l | awk '{print $1}' > /tmp/owofetch_brew_tmp", "(brew-cellar)"},
																			{"/usr/local/bin/brew", "find $(brew --caskroom 2>/dev/stdout) -maxdepth 1 -type d 2> /dev/null | wc -l | awk '{print $1}' > /tmp/owofetch_brew_tmp", "(brew-cask)"}};
#endif
#ifndef _WIN32
	...
	for (int i = 0; i < pkgman_count; i++) {
		...
		if (access(current->command_path, F_OK) != -1) {
			...
			FILE* fp = fopen("/tmp/owofetch_brew_tmp", "r");
	#endif
